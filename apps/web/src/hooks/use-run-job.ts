...
/**
 * Runs code against public cases and follows the result.
 *
 ...
 *
 * Practice Activities and the assessment workspace both run this way. The only
 * difference is the endpoint, and the rule that matters is the same in both:
 * a Run is never an attempt and never a grade.
 */
...
export function useRunJob(endpoint: string) {
  const [state, setState] = useState<RunJobState>({ phase: "idle" });
  ...
  useEffect(() => {
    ...
    const onStatus = (payload: SubmissionStatusPayload) => {
      // A SUBMIT is followed elsewhere, and a RUN whose id we do not recognise
      // belongs to a press the Coder has already moved past.
  ...
  const run = useCallback(
    async (input: { language: Language; sourceCode: string }) => {
      ...
      try {
        const { jobId } = await apiClient.post<{ jobId: string }>(endpoint, input);
